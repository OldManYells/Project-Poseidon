...
public class BlockPortal extends BlockBreakable {
    ...
    public boolean a_(World world, int i, int j, int k) {
        ...
        if (world.getTypeId(i - 1, j, k) == net.minecraft.server.CraftBlock.OBSIDIAN.id || world.getTypeId(i + 1, j, k) == net.minecraft.server.CraftBlock.OBSIDIAN.id) {
            b0 = 1;
        }

        if (world.getTypeId(i, j, k - 1) == net.minecraft.server.CraftBlock.OBSIDIAN.id || world.getTypeId(i, j, k + 1) == net.minecraft.server.CraftBlock.OBSIDIAN.id) {
            b1 = 1;
        }
        ...
        if (b0 == b1) {
            ...
            for (l = -1; l <= 2; ++l) {
                for (i1 = -1; i1 <= 3; ++i1) {
                    ...
                    if (l != -1 && l != 2 || i1 != -1 && i1 != 3) {
                        ...
                        if (flag) {
                            if (j1 != net.minecraft.server.CraftBlock.OBSIDIAN.id) {
                                return false;
                            } else {
                                blocks.add(bworld.getBlockAt(i + b0 * l, j + i1, k + b1 * l)); // CraftBukkit
                            }
                        } else if (j1 != 0 && j1 != net.minecraft.server.CraftBlock.FIRE.id) {
                            return false;
                        }
                    }
                }
            ...
            for (l = 0; l < 2; ++l) {
                for (i1 = 0; i1 < 3; ++i1) {
                    world.setTypeId(i + b0 * l, j + i1, k + b1 * l, net.minecraft.server.CraftBlock.PORTAL.id);
                }
            }
    ...
    public void doPhysics(World world, int i, int j, int k, int l) {
        ...
        if (world.getTypeId(i, i1 - 1, k) != net.minecraft.server.CraftBlock.OBSIDIAN.id) {
            ...
            if (j1 == 3 && world.getTypeId(i, i1 + j1, k) == net.minecraft.server.CraftBlock.OBSIDIAN.id) {
                ...
                if (flag && flag1) {
                    world.setTypeId(i, j, k, 0);
                } else if ((world.getTypeId(i + b0, j, k + b1) != net.minecraft.server.CraftBlock.OBSIDIAN.id || world.getTypeId(i - b0, j, k - b1) != this.id) && (world.getTypeId(i - b0, j, k - b1) != CraftBlock.OBSIDIAN.id || world.getTypeId(i + b0, j, k + b1) != this.id)) {
                    world.setTypeId(i, j, k, 0);
                }
            } else {
                world.setTypeId(i, j, k, 0);
