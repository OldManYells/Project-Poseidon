...
public class BlockDiode extends net.minecraft.server.CraftBlock {
    ...
    private boolean f(World world, int i, int j, int k, int l) {
        ...
        switch (i1) {
        case 0:
            return world.isBlockFaceIndirectlyPowered(i, j, k + 1, 3) || world.getTypeId(i, j, k + 1) == net.minecraft.server.CraftBlock.REDSTONE_WIRE.id && world.getData(i, j, k + 1) > 0;

        case 1:
            return world.isBlockFaceIndirectlyPowered(i - 1, j, k, 4) || world.getTypeId(i - 1, j, k) == net.minecraft.server.CraftBlock.REDSTONE_WIRE.id && world.getData(i - 1, j, k) > 0;

        case 2:
            return world.isBlockFaceIndirectlyPowered(i, j, k - 1, 2) || world.getTypeId(i, j, k - 1) == net.minecraft.server.CraftBlock.REDSTONE_WIRE.id && world.getData(i, j, k - 1) > 0;

        case 3:
            return world.isBlockFaceIndirectlyPowered(i + 1, j, k, 5) || world.getTypeId(i + 1, j, k) == CraftBlock.REDSTONE_WIRE.id && world.getData(i + 1, j, k) > 0;
