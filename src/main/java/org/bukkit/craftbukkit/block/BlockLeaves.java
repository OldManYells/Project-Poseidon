...
public class BlockLeaves extends BlockLeavesBase {
            ...
            for (int i1 = -b0; i1 <= b0; ++i1) {
                for (int j1 = -b0; j1 <= b0; ++j1) {
                    for (int k1 = -b0; k1 <= b0; ++k1) {
                        ...
                        if (l1 == net.minecraft.server.CraftBlock.LEAVES.id) {
                    ...
                    for (l1 = -b0; l1 <= b0; ++l1) {
                        for (i2 = -b0; i2 <= b0; ++i2) {
                            for (j2 = -b0; j2 <= b0; ++j2) {
                                ...
                                if (k2 == net.minecraft.server.CraftBlock.LOG.id) {
                                    this.a[(l1 + k1) * j1 + (i2 + k1) * b1 + j2 + k1] = 0;
                                } else if (k2 == net.minecraft.server.CraftBlock.LEAVES.id) {
    ...
    public int a(int i, Random random) {
        return net.minecraft.server.CraftBlock.SAPLING.id;
    }

    public void a(World world, EntityHuman entityhuman, int i, int j, int k, int l) {
        if (!world.isStatic && entityhuman.G() != null && entityhuman.G().id == Item.SHEARS.id) {
            entityhuman.a(StatisticList.C[this.id], 1);
            this.a(world, i, j, k, new ItemStack(CraftBlock.LEAVES.id, 1, l & 3));
        } else {
