...
public class BlockSoil extends net.minecraft.server.CraftBlock {
        ...
        if (random.nextInt(5) == 0) {
            if (!this.h(world, i, j, k) && !world.s(i, j + 1, k)) {
                ...
                if (l > 0) {
                    ...
                    world.setTypeId(i, j, k, net.minecraft.server.CraftBlock.DIRT.id);
                }
            } else {
    ...
    private boolean g(World world, int i, int j, int k) {
        ...
        for (int l = i - b0; l <= i + b0; ++l) {
            for (int i1 = k - b0; i1 <= k + b0; ++i1) {
                if (world.getTypeId(l, j + 1, i1) == net.minecraft.server.CraftBlock.CROPS.id) {
                    return true;
                }
            }
        }
