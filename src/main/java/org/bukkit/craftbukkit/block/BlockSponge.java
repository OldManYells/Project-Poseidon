...
public class BlockSponge extends net.minecraft.server.CraftBlock {
        ...
        for (int x = i - radius; x <= i + radius; ++x) {
            for (int y = j - radius; y <= j + radius; ++y) {
                ...
                for (int z = k - radius; z <= k + radius; ++z) {
                    ...
                    if ((type != net.minecraft.server.CraftBlock.WATER.id && type != CraftBlock.STATIONARY_WATER.id)) continue;
