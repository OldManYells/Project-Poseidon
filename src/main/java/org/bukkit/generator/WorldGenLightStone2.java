...
public class WorldGenLightStone2 extends WorldGenerator {
    ...
    public boolean a(World world, Random random, int i, int j, int k) {
        if (!world.isEmpty(i, j, k)) {
            return false;
        } else if (world.getTypeId(i, j + 1, k) != CraftBlock.NETHERRACK.id) {
            return false;
        } else {
            world.setTypeId(i, j, k, CraftBlock.GLOWSTONE.id);
            ...
            for (int l = 0; l < 1500; ++l) {
                ...
                if (world.getTypeId(i1, j1, k1) == 0) {
                    ...
                    for (int i2 = 0; i2 < 6; ++i2) {
                        ...
                        if (j2 == CraftBlock.GLOWSTONE.id) {
                            ++l1;
                        }
                    }

                    if (l1 == 1) {
                        world.setTypeId(i1, j1, k1, CraftBlock.GLOWSTONE.id);
                    }
                }
            }
