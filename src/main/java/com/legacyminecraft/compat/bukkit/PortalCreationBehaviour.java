...
public final class PortalCreationBehaviour {
    ...
    public boolean createPortal(org.bukkit.World bukkitWorld, Location location, Random random, int creationRadius) {
        World world = WORLD_HANDLE_BRIDGE_BEHAVIOUR.resolveWorldServerHandle(bukkitWorld);
            ...
            for (int sideOffset = -1; sideOffset <= 1; ++sideOffset) {
                for (int depthOffset = 1; depthOffset < 3; ++depthOffset) {
                    for (int yOffset = -1; yOffset < 3; ++yOffset) {
                        ...
                        addUniqueBlock(changedBlocks, bukkitWorld.getBlockAt(frameX, frameY, frameZ));
                    }
                }
            }
        }
        ...
        if (PORTAL_CREATE_EVENT_BRIDGE_BEHAVIOUR.shouldCancelPortalCreation(changedBlocks, bukkitWorld)) {
            return false;
        }
