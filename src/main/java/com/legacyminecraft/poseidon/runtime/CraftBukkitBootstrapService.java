...
/**
 * Canonical bootstrap flow for the legacy CraftBukkit main wrapper.
 */
public final class CraftBukkitBootstrapService {
    private static final CraftBukkitBootstrapService INSTANCE = new CraftBukkitBootstrapService();

    private CraftBukkitBootstrapService() {
    }

    public static CraftBukkitBootstrapService getInstance() {
        return INSTANCE;
    }

    public void run(String[] args, JlineModeController jlineModeController) {
        ...
        if ((options == null) || (options.has("?"))) {
            ...
            try {
                boolean useJline = !"jline.UnsupportedTerminal".equals(System.getProperty("jline.terminal"));
                jlineModeController.setUseJline(useJline);

                if (options.has("nojline")) {
                    System.setProperty("jline.terminal", "jline.UnsupportedTerminal");
                    System.setProperty("user.language", "en");
                    jlineModeController.setUseJline(false);
                }
    ...
    private static String resolveImplementationVersion() {
        Package bootstrapPackage = CraftBukkitBootstrapService.class.getPackage();
        if (bootstrapPackage == null) {
            return "unknown";
        }
        String implementationVersion = bootstrapPackage.getImplementationVersion();
